...
public class CrossKNNExperiment {

	private Map<Developer, SimpleSniffer> snifferMap;
	private Map<Integer, Developer> developers;

	public CrossKNNExperiment() throws IOException {
	...
	public void train(Class<? extends SimpleKNNSniffer> SnifferClass, Project trainProject, Project testProject,
			String smell, int devId) throws IOException, InstantiationException,
			IllegalAccessException {
		...
		Map<Node,NodeAnalysis> analysis = loadAnalysis(sniffer, trainProject.getAnalysisSource(smell),	trainProject, developer);
		List<NodeAnalysis> anls = new ArrayList<NodeAnalysis>(analysis.values());
		
		for (NodeAnalysis anl : anls) {
			sniffer.addAnalysis(anl);
		}
		...
		SimpleKNNSniffer sniffer2 = SnifferClass.newInstance();
		
		from += ClassifierEvaluator.getEvaluation(
				((SimpleKNNSniffer) sniffer2).getInitialRule(), anls);
		sniffer2.setBestClassifier(sniffer.getBestClassifier());
		to += ClassifierEvaluator.getEvaluation(
				sniffer2.getBestClassifier(), anls);
		
//		iterations += ((Rule)sniffer2.getBestClassifier()).getIterations(); 
		
		System.out.println(developer.getId() + "\t"
				+ from + "\t"
				+ to + "\t" 
//				+ iterations 
				+ " Analysis: "+ anls.size() + " - " + sniffer.getBestClassifier());
	}
