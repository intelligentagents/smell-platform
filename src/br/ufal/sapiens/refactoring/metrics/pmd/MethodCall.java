...
public class MethodCall {
	 ...
	 * primary expression.
	 */
	private MethodCall(ASTPrimaryExpression expression,
	 ...
	 */
	public static List<MethodCall> createMethodCalls(
			ASTPrimaryExpression expression) {
		List<MethodCall> result = new ArrayList<MethodCall>();

		if (isNotAConstructorCall(expression) && isNotLiteral(expression)
				&& hasSuffixesWithArguments(expression)) {
			ASTPrimaryPrefix prefixNode = expression
					.getFirstDescendantOfType(ASTPrimaryPrefix.class);
			MethodCall firstMethodCallInChain = new MethodCall(expression,
					prefixNode);
	...
	private static boolean isNotAConstructorCall(
			ASTPrimaryExpression expression) {
		return !expression
				.hasDescendantOfType(ASTAllocationExpression.class);
	}
	...
	private static List<ASTPrimarySuffix> findSuffixesWithoutArguments(
	...
	private static boolean hasSuffixesWithArguments(
			ASTPrimaryExpression expr) {
	...
	private void checkViolation() {
		...
		if (SCOPE_LOCAL.equals(baseScope)) {
			...
			if (lastAssignment != null && !lastAssignment.allocation
					&& !lastAssignment.iterator && !lastAssignment.forLoop) {
				violation = true;
	...
	private void determineType() {
		...
		try {
			ownerClassName = expression.getFirstParentOfType(ASTClassOrInterfaceDeclaration.class).getImage(); // By mario
		} catch (Exception e) {}
		...
		if (var == null) {
			...
			var = determineTypeOfVariable(baseName, scope
					.getEnclosingScope(MethodScope.class)
					.getVariableDeclarations().keySet());
		}
		if (var == null) {
		...
		if (var == null
				&& (THIS.equals(baseName) || SUPER.equals(baseName))) {
			baseScope = SCOPE_CLASS;
		}
	...
	private VariableNameDeclaration findInLocalScope(String name,
			Scope scope) {
		VariableNameDeclaration result = null;

		result = determineTypeOfVariable(name,
				scope.getDeclarations(VariableNameDeclaration.class)
						.keySet());
	...
	private VariableNameDeclaration determineTypeOfVariable(
			String variableName, Set<VariableNameDeclaration> declarations) {
		VariableNameDeclaration result = null;
	...
	private Assignment determineLastAssignment() {
		...
		ASTBlock block = expression.getFirstParentOfType(
				ASTMethodDeclaration.class).getFirstChildOfType(
				ASTBlock.class);

		List<ASTVariableDeclarator> variableDeclarators = block
		...
		for (ASTVariableDeclarator declarator : variableDeclarators) {
			...
			if (variableDeclaratorId.hasImageEqualTo(baseName)) {
				...
				assignments.add(new Assignment(declarator.getBeginLine(),
						allocationFound, iterator, forLoop));
			}
		}

		List<ASTAssignmentOperator> assignmentStmts = block
			...
			if (stmt.hasImageEqualTo(SIMPLE_ASSIGNMENT_OPERATOR)) {
				boolean allocationFound = stmt.jjtGetParent()
						.getFirstDescendantOfType(
								ASTAllocationExpression.class) != null;
				boolean iterator = isIterator();
				assignments.add(new Assignment(stmt.getBeginLine(),
						allocationFound, iterator, false));
			}
		}
	...
	private boolean isIterator() {
		...
		if ((baseType != null && baseType == Iterator.class)
				|| (baseTypeName != null && baseTypeName
						.endsWith("Iterator"))) {
			iterator = true;
		}
		return iterator;
	...
	public boolean isGetterSetter() {
		return methodName != null
				&& StringUtil.startsWithAny(methodName, "get", "is",
						"set");
	}
	
	public boolean isForeign() {
		return ownerClassName != baseTypeName; 
	}
