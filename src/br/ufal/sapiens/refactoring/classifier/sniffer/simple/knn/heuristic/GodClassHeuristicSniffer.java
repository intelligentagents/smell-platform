...
public class GodClassHeuristicSniffer extends SimpleHeuristicSniffer {
	...
	public void loadRules() {
		Rule rule1 = new Rule("GC - InnerClass (for ICPC - CountClassBase)", this.getSmell());
		Expression exp1 = new Expression("CountClassBase", Operator.GreaterEqualThan(), 2*1.0f);
		rule1.getExpressions().add(exp1);

		Rule rule2 = new Rule("GC - CountDeclMethodAll(NOM), CountLine(LOC)", this.getSmell());
		Expression exp21 = new Expression("CountDeclMethodAll", Operator.GreaterEqualThan(), 20*1.0f);
		Expression exp22 = new Expression("CountLine", Operator.GreaterEqualThan(), 500*1.0f);
		rule2.getExpressions().add(exp21);
		rule2.getExpressions().add(exp22);

		Rule rule3 = new Rule("GC - SumCyclomatic(WMC),CountClassCoupled(CBO)", this.getSmell());
		Expression exp31 = new Expression("SumCyclomatic", Operator.GreaterEqualThan(), 47*1.0f);
		Expression exp32 = new Expression("CountClassCoupled", Operator.GreaterEqualThan(), 20*1.0f);
